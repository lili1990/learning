...
SERVER_JMX_PORT=`sed '/http.jmxport/!d;s/.*=//' conf/application.conf | tr -d '\r'`
SERVER_DEBUG_PORT=`sed '/http.debugport/!d;s/.*=//' conf/application.conf | tr -d '\r'`
...
CLASS_PARAM="-Djetty.webappPath=$DEPLOY_DIR/webapp app.server.Server"
